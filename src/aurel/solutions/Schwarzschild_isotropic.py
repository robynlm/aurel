...
kappa = 8 * jnp.pi # Einstein's gravitational constant
...
def betaup3(t, x, y, z):
    ...
    Nx, Ny, Nz = jnp.shape(x)
    return jnp.zeros((3, Nx, Ny, Nz))
...
def gdown4(t, x, y, z, analytical=False):
    ...
    if analytical:
        ...
        ones = jnp.ones(jnp.shape(x))
        zeros = jnp.zeros(jnp.shape(x))
        return jnp.array([
...
def Kdown3(t, x, y, z):
    ...
    Nx, Ny, Nz = jnp.shape(x)
    return jnp.zeros((3, 3, Nx, Ny, Nz))

def Tdown4(t, x, y, z):
    """Returns the energy-stress tensor"""
    Nx, Ny, Nz = jnp.shape(x)
    return jnp.zeros((4, 4, Nx, Ny, Nz))
